...
enum AIType { WALKER, GUARD, PACER, SLIDER };
...
private:
    ...
    // for ai_pace()
    glm::vec3 m_min_position; // minimum position in the pacing range
    glm::vec3 m_max_position; // maximum position in the pacing range
    bool m_moving_right = true; // true if moving right, false if moving left
...
public:
    ...
    // for killing and being killed by player
    bool check_collision_top(Entity* other);
    void defeat();
    bool get_m_is_active();
    ...
    void ai_walk();
    void ai_pace(Entity* player);
    void ai_slide(Entity* player);
